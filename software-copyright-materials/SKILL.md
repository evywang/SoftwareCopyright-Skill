...
- 软件分两类处理：有用户界面（页面/图形界面）的软件生成操作手册；无用户界面的软件（嵌入式固件、驱动、库、后端服务、命令行工具等）不生成操作手册，改生成技术方案文档（设计说明书骨架），业务理解阶段判定 `manual_kind` 并经用户确认。
...
- 写申请表和操作手册/技术方案文档前，必须先形成模型研判后的 `草稿/业务理解.md/json`，理解软件业务、行业、目标用户、核心价值和操作流程。
- 脚本只能收集项目证据、校验字段和生成文件；行业判断、功能抽取、代码抽取选择、操作手册/技术方案文档结构必须由模型阅读项目后决定，不得依赖脚本关键字表或固定范本。
...
- 技术方案文档成稿应像真实软件的设计说明书：模块信息表、关键设计、功能设计和接口参数表必须来自真实项目设计；正文不得出现页面、按钮、点击等界面词汇；架构图、流程图、序列图使用可见的【图预留】文字占位。生成时同步输出 `草稿/技术方案文档自检记录.md/json`。
- 截图方式只允许用户选择（仅 operation 模式）：Playwright CLI 自动截图或用户自行截图。用户选择自动截图后，先检查固定验证版本的 `playwright-cli` 是否可用；如果用户说现在不截图、先跳过截图或截图失败，操作手册仍必须保留清晰可见的截图预留位置，正式 Word 中也要能看到。
...
- 生成技术方案文档（无界面软件）前读 [technical_design_structure.md](references/technical_design_structure.md)。
...
- `environment`：OfficeCLI 缺失时必须由用户确认全局安装并在安装后重启 coding agent；版本不匹配时，用户必须切换固定版本或明确承担使用未经验证版本的风险。
...
- `screenshot-method`（仅 operation 模式）：操作手册截图前，用户必须在 Playwright CLI 自动截图、用户自行截图两种方式中选择一种；如果用户明确说“现在不截图/先跳过截图”，记录为 `skip`。design 模式（技术方案文档）不记录该门禁。
...
- 图纸渲染（技术方案文档 DOT 图）是否可用，以及当前系统可用的渲染后端。
- 当前 OfficeCLI 路径、版本，以及是否等于固定验证版本 `1.0.151`。
- 如 OfficeCLI 缺失，询问用户是否使用官方命令全局安装；不得下载到项目目录或 skill 目录，也不得静默安装。
- 如官方全局安装已完成但当前进程尚未识别，必须要求用户重启 coding agent，停止当前执行，重启后重新运行环境检查。

用户选择：

- 如果用户愿意安装，按当前平台执行官方全局安装命令：Windows PowerShell 运行 `irm https://raw.githubusercontent.com/iOfficeAI/OfficeCLI/main/install.ps1 | iex`；macOS / Linux 运行 `curl -fsSL https://raw.githubusercontent.com/iOfficeAI/OfficeCLI/main/install.sh | bash`。安装完成后要求用户重启 coding agent；不得在当前任务中假定 PATH 已刷新后继续执行。
...
- 软件是否存在面向用户的图形界面/页面；有界面填 `manual_kind: operation`，无界面（嵌入式固件、驱动、库、后端服务、命令行工具等）填 `manual_kind: design`
- 应该重点读取哪些文档和源码
- 软件属于什么行业 / 领域
- 目标用户是谁
- 核心价值是什么
- 哪些功能应写入软著申请资料
- 典型操作流程如何组织
- 有界面软件的操作手册适合采用什么章节结构；无界面软件按技术方案文档骨架整理模块、关键设计、功能流程、接口和出错处理
- 申请表建议口径如何表达
...
其中 `manual_kind` 决定生成哪种文档：`operation` 时 `manual_modules` 是操作手册的核心输入，必须按真实页面、导航入口或业务流程填写；`design` 时不填 `manual_modules`，改填 `design_spec`（requirements、conditions、architecture、modules、overview、key_designs、functions、ui_statement、api_groups、error_handling，字段要求见 [business_understanding_rules.md](references/business_understanding_rules.md) 与 [technical_design_structure.md](references/technical_design_structure.md)），模块、功能、接口必须来自真实项目源码，不得编造。脚本不得按 `auth/query/form` 等分类模板自动补入口、步骤或反馈；缺少 `manual_modules`、`design_spec` 或关键字段时必须停止让模型回到项目证据中补写。`manual_modules` 每个模块必须包含：
...
生成 `业务理解.md/json` 后必须停止，等待用户确认或修改；确认内容必须包含文档类型（操作手册或技术方案文档）是否正确。业务理解确认前，不得生成申请表和操作手册/技术方案文档。如果业务理解仍不充分，先请用户补充产品说明。用户确认后运行：
...
- 总页数 `>= 60`：生成 `代码.md`（前30页+后30页合并为一份，共 60 页）。
- 总页数 `< 60` 且候选源码已用尽：生成 `代码.md`（全部代码材料）。
...
生成操作手册/技术方案文档草稿（同一脚本按业务理解的 `manual_kind` 自动分支）：
...
### 8. 选择并获取截图（仅 operation 模式）

`manual_kind=design`（无界面软件）没有截图阶段：不记录 `screenshot-method` 门禁、不运行截图工具，技术方案文档中的架构图/流程图/序列图由脚本写入可见的【图预留】文字占位，用户可在正式 Word 中自行插图。

`manual_kind=operation` 时，操作手册草稿完成后，先停止并让用户选择截图方式，必须给出两种选项：
...
- 选择 Playwright CLI 自动截图：必须先读取 [playwright_cli_screenshots.md](references/playwright_cli_screenshots.md)，运行其中的通用全局命令检查脚本，并按返回的可执行文件绝对路径、安装门禁、固定版本、后台服务、命名、浏览器会话和落盘校验规则执行。不得因裸命令不在 PATH 就要求重启 coding agent，也不得改用只能把截图显示在会话中的浏览器工具冒充成功；每张截图都必须是 `截图原始/` 下可读取且非空的本地图片文件。
...
- 软件名称和版本号是否一致
- 代码材料页眉软件名称是否与 `申请表信息.md` 的“软件全称”一致
- 代码材料页眉版本号是否与 `申请表信息.md` 的“版本号”一致
- 操作手册/技术方案文档 Word 页眉是否与代码材料页眉一致，均使用 `申请表信息.md` 的“软件全称”和“版本号”
- `业务理解.md` 是否准确反映软件真实业务、行业和目标用户
- operation 模式：操作手册是否符合审核员阅读场景，普通读者是否能看懂模块用途和操作方式；每个章节是否有段落内容，核心模块是否写清模块用途、操作过程和结果反馈，是否避免过度技术化语言；截图是否正确；若用户跳过截图，正式操作手册是否保留可见截图预留位置
- design 模式：技术方案文档的模块表、关键设计、功能设计和接口表是否与项目源码一致，接口能否回溯到真实代码；正文是否残留页面、按钮等界面词汇；声明的图纸是否已渲染并嵌入正文，未渲染的【图预留】位置是否可见；agent 已逐张查看渲染图，确认无文字重叠、同级节点对齐一致、比例适配竖版 Word
- `申请表信息.md` 中“待用户确认”的字段是否已确认
- 代码材料是否只来自用户确认的完整文件

用户确认后，必须记录 `markdown` 门禁；未记录时不得生成正式 Word/TXT。

确认脚本会为业务理解、代码选择、申请表和最终草稿记录内容指纹。任何已确认文件在确认后又发生修改，对应门禁立即失效，必须让用户查看变化并重新确认；不能沿用旧的确认布尔值。最终确认前还必须检查代码提取清单、当前代码 Markdown、申请表、操作手册/技术方案文档及其自检记录全部存在，且没有与当前代码输出模式冲突的旧草稿。
...
- `正式资料/申请表信息.txt`
- `正式资料/<软件全称>-代码.docx`（前30页+后30页合并为一份；不足 60 页时为全部代码材料）
- operation 模式：`正式资料/<软件全称>_操作手册.docx`
- design 模式：`正式资料/<软件全称>_技术方案文档.docx`
- `正式资料/生成报告.md`
...
`validate` 只证明 OpenXML 结构可读，不能证明分页正确。Windows 且安装 Word 时由 OfficeCLI 使用 `stats --page-count` 读取自动分页后的真实页数；前 30 页+后 30 页合并文档必须正好 60 页，否则生成失败并要求重新校准选材量，不得改回人工固定分页。其他环境先用 OfficeCLI HTML 预览，再用 Word/WPS 打开最终文件复核页数。只使用 OfficeCLI 后端，不引入其他 DOCX 渲染依赖。
...
- 多个项目候选目录需要选择。
- 启动环境检查发现 OfficeCLI 缺失时，询问用户是否全局安装；安装完成后要求重启 coding agent 并停止，重启后再继续。版本不匹配时询问是否切换固定版本。
